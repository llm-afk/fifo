...
static inline void fifo_init(fifo_t *fifo, uint8_t *buf, uint32_t buf_len)
{
    fifo->buf = buf;
...
static inline uint32_t fifo_getUsed(fifo_t *fifo)
{
    return fifo->write - fifo->read;
}

static inline uint32_t fifo_getAvai(fifo_t *fifo)
{
    return fifo->buf_len - (fifo->write - fifo->read);
}

static inline void fifo_write(fifo_t *fifo, const uint8_t *src, uint32_t len)
{
...
static inline void fifo_read(fifo_t *fifo, uint8_t *dst, uint32_t len)
{
...
static inline uint8_t fifo_peek(fifo_t *fifo)
{
    return fifo->buf[FIFO_MASK(fifo, fifo->read)];
}

static inline void fifo_drop(fifo_t *fifo, uint32_t len)
{
    fifo->read += len;
}
